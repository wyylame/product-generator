...
// сохраняе категории на страницу
...
function addCategoriesOnPage() {
  ...
  categories.map(
    (e) => (categoriesList.innerHTML += categoryStructure(e.title, e.name))
  );
}
...
function addCategoryBtnsOnProduct() {
  ...
  categories.map(
    (item) =>
      (productCategoryContainer.innerHTML += categoryProductStructure(
        item.title,
        item.name
      ))
  );
...
function addParamsOnPage() {
  ...
  params.map(
    (item, i) =>
      (paramsContainer.innerHTML += paramStructure(i + 1, item.key, item.value))
  );
}
...
function pushProduct() {
  const currentCategory = [
    ...productCategoryContainer.querySelectorAll("input"),
...
categoriesList.addEventListener("click", (e) => {
  if (e.target.classList.contains("remove")) {
    console.log(e.target.parentNode.querySelector("h4").innerText);
    categories.splice(
      categories.findIndex(
        (item) =>
          item.title === e.target.parentNode.querySelector("h4").innerText
      ),
      1
    );
    saveToLocalStorageCategories();
